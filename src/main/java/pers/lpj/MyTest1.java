...
public class MyTest1 {
    ...
    /**
     * JVM 由几个部分组成   加载器子系统，运行时数据区，执行引擎，本地方法接口 组成 .
     * <p>
     ...
     * *      a 根(启动类加载)类加载器 （Bootstrap ClassLoader）<没有父加载器，其他的加载器有且只有一个父加载器> rt.jar
     *           负责加载系统类库（例如 java.lang） <由c++实现 ，当获取这个classLoader时，返回结果为Null (和下面两个类加载器有区别)>
     * *      b 扩展类加载器 （Extention ClassLoader）<父加载器是根类加载器>: jre包下ext 或者自定义指定<由java实现>
     * *      c  应用（系统）类加载器 （System ClassLoader）<父加载器是扩展类加载器> classpath 或者自定义<由java实现>
     * *
     ...
     * * 要在程序首次使用时报告此错误
     * <p>
     * 2 连接 <完成类的 <验证 准备 解析> 工作> (将已经加载到内存的类二进制文件合并到虚拟机的运行时环境当中)
     *      a. 验证 完成对字节码文件的校验（是否符合java字节码规范<魔数·等等>）
